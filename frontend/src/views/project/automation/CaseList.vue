    ...
    <section>
      <el-card class="box-card" style="margin-left: 20px">
        <div slot="header" class="clearfix">
            <span class="h1" style=" font-weight:bold">默认用例</span>
        </div>
        <!--列表-->
        <el-select v-model="url"  placeholder="测试环境" style="float: left">
            <el-option v-for="(item,index) in Host" :key="index+''" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <el-checkbox v-model="checked">同步测试</el-checkbox>
      ...
      <el-card class="box-card" style="margin-left: 20px">
        <!--工具条-->
        <div slot="header" class="clearfix">
            <span class="h1" style=" font-weight:bold">自定义用例</span>
        </div>
